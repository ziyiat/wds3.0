...
(function($) {
	...
	// DOM elements
	overlay, closeBtn, image, prevBtn, nextBtn, leftBtn, rightBtn, bottom, caption, nav, number, imgnavi,
	...
	*/
	...
	$(document).ready(function() {
		$(document.body).append(
		 ...
		 //$('#rollpic').clone().appendTo('div.imgnavi');
		 $([imgnavi]).append([
		 	$('<ul id="rollnav" />').append([
		 		$('<li />').append([$('<img id=imgleft />')[0]]).click(previous)[0],
		 		$('<li />').append([$('<img id=imgnow />')[0]]).click(function(){return false;})[0],
		 		$('<li />').append([$('<img id=imgright />')[0]]).click(next)[0]
		 		])[0]
		 	]);
	...
	*/
	...
	function showImage(noAnim) {
		...
		/*----------------------------*/
		$('#rollnav img').attr("src","");
		$("#imgnow").attr("src",activeURL);    
		// console.log(prevImage);
		$("#imgleft").attr("src", images[prevImage][0]);
		$('#imgright').attr("src",images[nextImage][0]);
		/*----------------------------*/
	...
	function scrollZoom(e, delta) {
		...
		// console.log(to+" currentSize="+currentSize+"  delta="+delta);
